...
public class backend {
    ...
    public boolean makeMove(int moveIndex, int player) {
    ...
    // returns arrayList of victory moves (<24 lines)
    public ArrayList<Integer> getVictoryMoves() {
        ArrayList<Integer> winMoves = new ArrayList<Integer>(3);
        for (int boardIndex = 0; boardIndex < 76; ++boardIndex) {
            if (gameBoard[wins[boardIndex][0]] != 0 && 
            gameBoard[wins[boardIndex][0]] == gameBoard[wins[boardIndex][1]] &&
            gameBoard[wins[boardIndex][0]] == gameBoard[wins[boardIndex][2]] &&
            gameBoard[wins[boardIndex][0]] == gameBoard[wins[boardIndex][3]]) 
            {
                winMoves.add(wins[boardIndex][0]);
                winMoves.add(wins[boardIndex][1]);
                winMoves.add(wins[boardIndex][2]);
                winMoves.add(wins[boardIndex][3]);
            }
        }
        return winMoves;
    }
    ...
    public void retrieveScores() throws FileNotFoundException, UnsupportedEncodingException {
        try {
            File file = new File ("Scores.txt");
            Scanner scan = new Scanner (file);
            if(!(file.exists() && file.length() == 0)) {
                int numScores = scan.nextInt();
                Score[] scores2 = new Score[numScores+1];
                int index = 0;
                while (scan.hasNext() && scan.hasNextLine()) {
                    scores2[index] = new Score(scan.next(), scan.nextInt());
                    index++;
                }
                scores2[numScores] = new Score("Not a score", -1);
                scan.close();
                Arrays.sort(scores2, (a,b) -> -a.compareTo(b));
                scores = scores2;
            }
            else { 
                initializeScoreFile();
    ...
    // initializes score file (Score.txt) (<24 lines)
    public void initializeScoreFile() throws FileNotFoundException, UnsupportedEncodingException {
    	 PrintWriter writer = new PrintWriter("Scores.txt", "UTF-8");
         writer.println("5");
         writer.println("null -100");
         writer.println("null -100");
         writer.println("null -100");
         writer.println("null -100");
         writer.println("null -100");
    ...
    public void addPlayerInitials(String s) {
        currentPlayer = s;
    	...
    	if(firstTime) {
            scores[scores.length-1] = new Score(s, 0);
    	}
    }
    
    // gets player score (<24 lines)
    public String getPlayerScore(String initial) {
    	for(Score sc : scores) {
            if(sc.name.equals(initial)) {
                return "" + sc.wins;
            }
    	}
    	return "";
    ...
    public void writeScores() throws FileNotFoundException, UnsupportedEncodingException {
        ...
        for(Score sc : scores) {
            if(sc.wins != -1)
            writer.println(sc.name + " " + sc.wins);
        }
        writer.close();
    }
    ...
    public void incrementScore(String name) {
        System.out.print("incrementScore called");
        boolean incremented = false;
        for (Score sc : scores) {
            if (sc.name.equals(name) && !incremented) {
                sc.increment();
            }
        }
        Arrays.sort(scores, (a,b) -> -a.compareTo(b));
